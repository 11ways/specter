...
Tab.setMethod(function init() {
	...
	this.browser_interface.afterOnce('ready', async function afterReady() {
		...
		try {
			// Create a new page
			page = await that.browser_interface.browser_instance.newPage();
		} catch (err) {
			console.error('Error initializing tab', that);
			throw err;
		}

		// Disable the navigation timeout (is set to 30s by default)
...
Tab.setAfterMethod('ready', async function goto(url) {
	...
	let response_url = await tab.url();
 ...
 * @version  0.1.1
 */
Tab.setMethod(async function close() {

	let result;

	try {
		result = await this.tab_instance.close();
	} catch (err) {
		// Already closed?
	}
