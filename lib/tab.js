...
var Tab = Fn.inherits('Informer', 'Develry.Specter', function Tab(browser, tab_interface) {
	...
	// The url we initially requested
	this.start_url = null;

	// The url we're currently requesting
...
Tab.setMethod(function goto(url) {
	...
	this.browser.page_queue.add(function doGoto(done) {
		let thennable = that._goto(url);
...
Tab.setMethod(function getLinks(callback) {
	...
	this.evaluate(function() {
		...
		if (canonical) {
			canonical = canonical.href || canonical.getAttribute('href');
		}
