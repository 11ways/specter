...
var Crawler = Fn.inherits('Informer', 'Develry.Specter', function Crawler(browser) {
	...
	// The URLs that are forbidden (including underlying paths)
	this.forbidden_urls = [];
...
Crawler.decorateMethod(Blast.Decorators.loopOverArgument(), function addStartUrl(url) {
	...
	if (this.arrayContainsUrl(this.start_urls, url)) {
		return;
	}
 ...
 * @param    {String|RURL}   url
 */
Crawler.decorateMethod(Blast.Decorators.loopOverArgument(), function addForbiddenUrl(url) {

	url = Blast.Classes.RURL.parse(url);

	if (this.arrayContainsUrl(this.forbidden_urls, url)) {
		return;
	}

	this.forbidden_urls.push(url);
});
 ...
 * @param    {String}   url
 */
Crawler.decorateMethod(Blast.Decorators.loopOverArgument(), function addCanonical(url) {

	url = Blast.Classes.RURL.parse(url);

	if (this.arrayContainsUrl(this.canonical_urls, url)) {
		return;
	}

	this.canonical_urls.push(url);
});

/**
 * Is the given url already in the given array?
 *
 * @author   Jelle De Loecker   <jelle@elevenways.be>
 * @since    0.1.2
 * @version  0.1.2
 ...
 * @return   {Boolean}
 */
Crawler.setMethod(function arrayContainsUrl(arr, url) {
...
Crawler.setMethod(function isUrlAllowed(url, text) {
	...
	for (i = 0; i < this.start_urls.length; i++) {
		...
		// Make sure the given url is a descendant of this start url
		if (url.isDescendant(this.start_urls[i])) {
			allowed = true;
			break;
		}
	}

	if (allowed && this.forbidden_urls.length) {

		for (i = 0; i < this.forbidden_urls.length; i++) {

			// Make sure the given url is NOT a descendant of this forbidden url
			if (url.isDescendant(this.forbidden_urls[i])) {
				allowed = false;
				break;
			}
		}
...
Crawler.setMethod(function hasBeenQueued(url) {
	...
	let result = info.finished || info.queued;

	if (result) {
		return true;
	}

	let canonical = this.matchCanonicalUrl(url);

	// If this url matches a canonical url,
	// see if that canonical url has been queued
	if (canonical) {
		info = this.getPageInfo(canonical);
...
/**
 * Get the matching canonical url
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.1.2
 * @version  0.1.2
 ...
 * @return   {RURL|Boolean}
 */
Crawler.setMethod(function matchCanonicalUrl(url) {

	if (!this.canonical_urls.length) {
		return false;
	}
	...
	for (canonical of this.canonical_urls) {

		if (!url.isDescendant(canonical)) {
			continue;
		}

		// Make sure the segments match
		if (url.segments.join('/') != canonical.segments.join('/')) {
			continue;
		}

		// If the canonical url contains parameters, see if they match
		if (canonical.search) {
			let key;

			for (key in canonical.query) {
				if (url.param(key) != canonical.param(key)) {
					return false;
				}
			}
		}
...
Crawler.setMethod(async function _crawlPage(page, level, callback) {
	...
	page.getLinks(function gotLinks(err, result) {
		...
		if (result.canonical) {
			let canonical = that.normalizeUrl(result.canonical);
			that.markFinished(canonical);
			that.addCanonical(canonical);
		}
