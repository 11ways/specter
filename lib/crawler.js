 ...
 * @version  0.1.2
 */
Crawler.setMethod(function isUrlAllowed(url) {

	if (!this.ignore_extensions || !this.ignore_extensions.length) {
		return true;
	}
	...
	if (this.ignore_parameters && this.ignore_parameters.length) {
		...
		for (i = 0; i < params.length; i++) {
			...
			if (!this.parameterIsAllowed(param)) {
				if (param != 'locale') {
					console.log('Removing parameter', param, 'from ' + url);
				}
